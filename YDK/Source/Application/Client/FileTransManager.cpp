...
{
	////promote client
	//XString filePath = TEXT("D:\\");
	//filePath += pMsg->m_strFileName;
	//SendFileAck(pMsg->m_unSenderID, pMsg->m_unLocalFileNO, filePath, pMsg->m_unFileTotalLength);

	//promote client
	static uint32 i = 0;
	XString fileName = XString().Format(TEXT("D:\\%s%d"),pMsg->m_strFileName , i);
	SendFileAck(pMsg->m_unSenderID, pMsg->m_unLocalFileNO, fileName, pMsg->m_unFileTotalLength);
	++i;
}
