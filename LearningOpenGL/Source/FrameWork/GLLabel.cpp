	...
	: m_colorTexUnit(0)
{
	...
	m_pTexData = new char[128 * 128];
	memset(m_pTexData, 0, 128 * 128);
...
void CGLLabel::Render()
{
	...
	if(glyphIndex)
	{
		if (!FT_Load_Glyph(m_fontFace, glyphIndex, FT_LOAD_RENDER))
		{
			...
			//letter.m_vVertex[1].m_UV.x = bitmapWidth / 128.0f;
			...
			//letter.m_vVertex[2].m_UV.x = bitmapWidth / 128.0f;
			//letter.m_vVertex[2].m_UV.y = bitmapHeight / 128.0f;
			...
			//letter.m_vVertex[3].m_UV.y = bitmapHeight / 128.0f;
			letter.m_vVertex[3].m_UV.y = 1;
			letter.m_vVertex[3].m_color = Color4F::WHITE;

			letter.m_vVertexIndex[0] = 0;
			letter.m_vVertexIndex[1] = 1;
			letter.m_vVertexIndex[2] = 3;

			letter.m_vVertexIndex[3] = 1;
			letter.m_vVertexIndex[4] = 2;
			letter.m_vVertexIndex[5] = 3;

			glBindBuffer(GL_ARRAY_BUFFER, m_vertexDataObj);
			glBufferData(GL_ARRAY_BUFFER, sizeof(letter.m_vVertex), &letter.m_vVertex[0], GL_STATIC_DRAW);

			glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_vertexIndexObj);
			glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(letter.m_vVertexIndex), &letter.m_vVertexIndex[0], GL_STATIC_DRAW);
			...
			glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_vertexIndexObj);
			glDrawElements(GL_TRIANGLES, sizeof(letter.m_vVertexIndex), GL_UNSIGNED_SHORT, 0);
